...
class mydsp : public dsp {
  ...
  // global data inserted here. -Kjetil.
	...
	virtual void buildUserInterface(UI* interface) {
     ...
     // Steve Harris am_pitchshift LADSPA code below:
  
     virtual void compute(int sample_count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
	unsigned long pos;
	fixp16 om;
        LADSPA_Data *input=inputs[0];
        LADSPA_Data *output=outputs[0];

	LADSPA_Data pitch = fhslider2;

	/* Buffer size (float value) */
	LADSPA_Data size = fhslider1;
